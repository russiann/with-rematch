(function (global, factory) {
	...
	*/
	...
	var createReducer = function (model) { return function (state, action) {
	  if ( state === void 0 ) state = model.state;

	  var reducer = model.reducers[action.type];
	  return reducer ? reducer(state, action.payload, action.props) : state;
	}; };

	var createActions = function (model, dispatch, props) {
	  return Object.keys(model.reducers || []).reduce(
	    function (actions, type) {
	      var obj;

	      return (Object.assign({}, actions,
	      ( obj = {}, obj[type] = function (payload) { return dispatch({type: type, payload: payload, props: props}); }, obj )));
	  },
	    {}
	  );
	};
